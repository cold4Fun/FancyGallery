...
class P3BaseTest {
    ...
    fun bottomNavigationBasics() {
        ...
        onView(withId(R.id.photo_grid)).check(matches(withEffectiveVisibility(Visibility.VI
                SIBLE)))
        onView(withId(R.id.map_view)).check(doesNotExist())
        onView(withId(R.id.map_fragment)).perform(click())

        onView(withId(R.id.map_view)).check(matches(withEffectiveVisibility(Visibility.VISI
                BLE)))
    ...
    fun galleryFirstPlaceholder() {
        waitFor(withId(R.id.photo_grid), atPosition(0,
            matchesDrawable(R.drawable.ic_placeholder)))
    }
    @Test
    fun galleryFirstImageLoaded() {
        waitFor(
            withId(R.id.photo_grid), atPosition(0,
                not(matchesDrawable(R.drawable.ic_placeholder)))
        )
    }
    @Test
    ...
    fun webFromGalleryFourHasProgressBarSubtitle() {
        ...
        onView(withId(R.id.web_view)).check(matches(withEffectiveVisibility(Visibility.VISI
                BLE)))
    ...
    fun galleryAfterWebNoSubtitleCachedImage() {
        ...
        waitFor(withId(R.id.web_view), webViewLoaded(), maxRepeat = 3 *
    ...
    fun mapClickMarkerShowsInfoAndRaises() {
        ...
        onView(withId(R.id.map_view)).perform(zoomTo(14.0)).perform(panTo(pos)).perform(cli
                ck())
        for (n in 0 until 50) {
    ...
    fun mapRetainsState() {
        ...
        onView(withId(R.id.map_view)).perform(zoomTo(14.0)).perform(panTo(firstMarker.posit
                ion))
        waitFor(withId(R.id.map_view), loadingIsComplete())
        onView(withId(R.id.gallery_fragment)).perform(click())
        onView(withId(R.id.map_view)).check(doesNotExist())
    ...
    private fun loadedMarkerCountMinimum(num: Int = MINIMUM_MARKER_COUNT):
            BoundedMatcher<View, MapView> {
    ...
    private fun loadingIsComplete() = object : BoundedMatcher<View,
            MapView>(MapView::class.java) {
        override fun describeTo(description: Description?) {
    ...
    private fun isFullyZoomedOut() = object : BoundedMatcher<View,
            MapView>(MapView::class.java) {
        override fun describeTo(description: Description?) {
            description?.appendText("is fully loaded")
        }
    ...
    private fun isFullyZoomedIn() = object : BoundedMatcher<View,
            MapView>(MapView::class.java) {
        override fun describeTo(description: Description?) {
    ...
    private fun matchesDrawable(resourceID: Int): Matcher<View?> {
        return object : BoundedMatcher<View?, ImageView>(ImageView::class.java) {
            ...
            override fun matchesSafely(imageView: ImageView): Boolean {
                val expBM = imageView.context.resources.getDrawable(resourceID,
                    null).toBitmap()
    ...
    private fun recyclerChildCount(num: Int = RECYCLER_ITEM_COUNT): Matcher<View?>
    {
        return object : BoundedMatcher<View?,
                RecyclerView>(RecyclerView::class.java) {
            override fun describeTo(description: Description?) {
                description?.appendText("RecyclerView having exactly $num
                        children")
            }
    ...
    private fun webViewLoaded() = object : BoundedMatcher<View?,
            WebView>(WebView::class.java) {
        override fun describeTo(description: Description?) {
    ...
    private fun getZoomLevel(matcher: Matcher<View>): Double {
        ...
        onView(matcher).perform(object : ViewAction {
            override fun getConstraints(): Matcher<View> =
                isAssignableFrom(MapView::class.java)
    ...
    private fun noopDelayAction(millis: Long) = object : ViewAction {
        ...
        override fun getDescription(): String = "Intentionally does nothing but
        delay"
        override fun perform(uiController: UiController?, view: View?) =
            sleep(millis)
    }
    private fun waitFor(
        ...
        onView(target).perform(repeatedlyUntil(noopDelayAction(sleepMillis),
            matcher, maxRepeat))
    }
}